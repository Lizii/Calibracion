<?php
	...
	if ($objCon->conectar()==true)
	{
		...
		*/
		...
		$DP=strtoupper($_POST['descripcionparte']);
		$DESP=strripos($DP, "'");

		if($DESP===false)
			$DESCRIPCIONPARTE=$DP;
		else
			$DESCRIPCIONPARTE=str_replace("'", "''", $DP);

		$PIEZASORDEN=$_POST['piezasorden'];
		$PIEZASRECHAZADAS=$_POST['piezasrechazadas'];

		$DF=strtoupper($_POST['descripcionfalla']);
		$DESF=strripos($DF, "'");

		if($DESF===false)
			$DESCRIPCIONFALLA=$DF;
		else
			$DESCRIPCIONFALLA=str_replace("'", "''", $DF);
		...
		{
			$RECHAZO='AD004';
			$RECHAZODESCRIPCION='ROSCA DAÑADA';
			$LOCALIZADEFECTO='ALMDR';
			$LOCALIZADEFECTODESC='AL FAB DRILLING / FAB AL PERFORADO';
		}
		if($_POST['origen_codigo']=="AD005")
		{
			$RECHAZO='AD005';
			$RECHAZODESCRIPCION=strtoupper($_POST['origen_otro']);
			$LOCALIZADEFECTO='ALMDR';
		...
		{
			$RECHAZO='FA015';
			$RECHAZODESCRIPCION='TORNILLO BARRIDO';
			$LOCALIZADEFECTO='FASSY';
			$LOCALIZADEFECTODESC='FINAL ASSEMBLY / ENSAMBLE FINAL';
		}
		if($_POST['origen_codigo']=="FA016")
		{
			$RECHAZO='FA016';
			$RECHAZODESCRIPCION='TORNILLO CAPADO';
			$LOCALIZADEFECTO='FASSY';
			$LOCALIZADEFECTODESC='FINAL ASSEMBLY / ENSAMBLE FINAL';
		}
		if($_POST['origen_codigo']=="FA017")
		{
			$RECHAZO='FA017';
			$RECHAZODESCRIPCION='MOTION SENSOR DAÑADO';
			$LOCALIZADEFECTO='FASSY';
			$LOCALIZADEFECTODESC='FINAL ASSEMBLY / ENSAMBLE FINAL';
		}
		if($_POST['origen_codigo']=="FA018")
		{
			$RECHAZO='FA018';
			$RECHAZODESCRIPCION='PERFORACION DAÑADA';
			$LOCALIZADEFECTO='FASSY';
			$LOCALIZADEFECTODESC='FINAL ASSEMBLY / ENSAMBLE FINAL';
		}
		if($_POST['origen_codigo']=="FA019")
		{
			$RECHAZO='FA019';
			$RECHAZODESCRIPCION='BALASTRO DAÑADO';
			$LOCALIZADEFECTO='FASSY';
			$LOCALIZADEFECTODESC='FINAL ASSEMBLY / ENSAMBLE FINAL';
		}
		if($_POST['origen_codigo']=="FA020")
		{
			$RECHAZO='FA020';
			$RECHAZODESCRIPCION='SURGE PROTECTOR DAÑADO';
			$LOCALIZADEFECTO='FASSY';
			$LOCALIZADEFECTODESC='FINAL ASSEMBLY / ENSAMBLE FINAL';
		}
		if($_POST['origen_codigo']=="FA021")
		{
			$RECHAZO='FA021';
			$RECHAZODESCRIPCION='VIDRIO/ACRILICO DAÑADO';
			$LOCALIZADEFECTO='FASSY';
			$LOCALIZADEFECTODESC='FINAL ASSEMBLY / ENSAMBLE FINAL';
		}
		if($_POST['origen_codigo']=="FA022")
		{
			$RECHAZO='FA022';
			$RECHAZODESCRIPCION='VIDRIO/ACRILICO FUERA DE ESPECIFICACION';
			$LOCALIZADEFECTO='FASSY';
			$LOCALIZADEFECTODESC='FINAL ASSEMBLY / ENSAMBLE FINAL';
		}
		if($_POST['origen_codigo']=="FA023")
		{
			$RECHAZO='FA023';
			$RECHAZODESCRIPCION=strtoupper($_POST['origen_otro']);
			$LOCALIZADEFECTO='FASSY';
		...
		if($_POST['origen_codigo']=="LD013")
		{
			$RECHAZO='LD013';
			$RECHAZODESCRIPCION='DIODO DAÑADO DURANTE ENSAMBLAJE';
			$LOCALIZADEFECTO='LEDAS';
			$LOCALIZADEFECTODESC='LED ASSEMBLY / ENSAMBLE LED';
		}
		if($_POST['origen_codigo']=="LD014")
		{
			$RECHAZO='LD014';
			$RECHAZODESCRIPCION='CLUSTER DAÑADO DURANTE ENSAMBLAJE';
			$LOCALIZADEFECTO='LEDAS';
			$LOCALIZADEFECTODESC='LED ASSEMBLY / ENSAMBLE LED';
		}
		if($_POST['origen_codigo']=="LD015")
		{
			$RECHAZO='LD015';
			$RECHAZODESCRIPCION=strtoupper($_POST['origen_otro']);
			$LOCALIZADEFECTO='LEDAS';
    ...
    $foto4=$_FILES['foto4']['name'];		  	   												   // Obtenemos el nombre de la foto1 	
	$foto5=$_FILES['foto5']['name'];          	  												   // Obtenemos el nombre de la foto2 	
	$foto6=$_FILES['foto6']['name'];		  	  												   // Obtenemos el nombre de la foto3 	

	$ruta_foto4=$_FILES['foto4']['tmp_name']; 	   												   // Guarda la foto1 en una carpeta temporalmente	
	$ruta_foto5=$_FILES['foto5']['tmp_name']; 	  												   // Guarda la foto1 en una carpeta temporalmente	
	$ruta_foto6=$_FILES['foto6']['tmp_name']; 	   												   // Guarda la foto1 en una carpeta temporalmente	  

	$destino_foto4="../fotos/".$foto4;		  	   												   // Generamos la dirección donde se guardará la foto4
	$destino_foto5="../fotos/".$foto6;		 	  												   // Generamos la dirección donde se guardará la foto5
	$destino_foto6="../fotos/".$foto6;		 	  											       // Generamos la dirección donde se guardará la foto6

	$foto4_nuevo=time().rand().".jpg";		       												   // Generamos un nuevo nombre aleatorio para la foto4
	$foto5_nuevo=time().rand().".jpg";		       												   // Generamos un nuevo nombre aleatorio para la foto5
	$foto6_nuevo=time().rand().".jpg";		  	  												   // Generamos un nuevo nombre aleatorio para la foto6

	$destino_foto4_nuevo="../fotos/".$foto4_nuevo; 												   // Generamos la dirección donde se guardará la foto4_nuevo
	$destino_foto5_nuevo="../fotos/".$foto5_nuevo; 												   // Generamos la dirección donde se guardará la foto5_nuevo
	$destino_foto6_nuevo="../fotos/".$foto6_nuevo; 												   // Generamos la dirección donde se guardará la foto6_nuevo

	if($foto4 && $foto5 && $foto6!=NULL) // Si las variables $foto4, $foto5 y $foto6 contienen imagenes, las tres imágenes se agregarán al archivo pdf
	{
		copy($ruta_foto4, $destino_foto4); 														   // Guarda la foto4 en el servidor
		copy($ruta_foto5, $destino_foto5); 														   // Guarda la foto5 en el servidor
		copy($ruta_foto6, $destino_foto6); 														   // Guarda la foto6 en el servidor

		$obj_simpleimage->load($destino_foto4);													   // Carga la foto1
		$obj_simpleimage->resize($obj_simpleimage->getWidth()/4, $obj_simpleimage->getHeight()/4); // Redimensiona la imágen a 1/4 de su tamaño real
		$obj_simpleimage->save($destino_foto4_nuevo);											   // Guarda los cambios realizados							

		if($obj_simpleimage->getWidth()>$obj_simpleimage->getHeight())							   // Si el ancho de la foto1 es más grande que el alto ... 
		{																						   
			$pdf->Image($destino_foto4_nuevo, 15.3,  200, 60); 									   // ... Agrega la foto1 en el archivo pdf en forma horizontal
		}					
		else                                                                                       // De no ser así ...
		{
			$pdf->Image($destino_foto4_nuevo, 30,  191.3, 30);                                     // ... Agrega la foto1 en el archivo pdf en forma vertical
		}

		$obj_simpleimage->load($destino_foto5);													   // Carga la imágen foto2
		$obj_simpleimage->resize($obj_simpleimage->getWidth()/4, $obj_simpleimage->getHeight()/4); // Redimensiona la imágen a 1/4 de su tamaño real
		$obj_simpleimage->save($destino_foto5_nuevo);											   // Guarda los cambios realizados	

		if($obj_simpleimage->getWidth()>$obj_simpleimage->getHeight())							   // Si el ancho de la foto2 es más grande que el alto ... 
		{
			$pdf->Image($destino_foto5_nuevo, 79.7,  200, 60);	 								   // ... Agrega la imágen foto2 en el archivo pdf en forma horizontal
		}
		else                                                                                       // De no ser así ...
		{
			$pdf->Image($destino_foto5_nuevo, 95,  191.3, 30);                                     // ... Agrega la imágen foto2 en el archivo pdf en forma vertical
		}

		$obj_simpleimage->load($destino_foto6);                                                    // Carga la imágen foto2
		$obj_simpleimage->resize($obj_simpleimage->getWidth()/4, $obj_simpleimage->getHeight()/4); // Redimensiona la imágen a 1/4 de su tamaño real
		$obj_simpleimage->save($destino_foto6_nuevo);                                              // Guarda los cambios realizados	
	 	
		if($obj_simpleimage->getWidth()>$obj_simpleimage->getHeight())							   // Si el ancho de la foto3 es más grande que el alto ... 
		{
			$pdf->Image($destino_foto6_nuevo, 144.4, 200, 60);	 								   // ... Agrega la imágen foto3 en el archivo pdf en forma horizontal
		}
		else                                                                                       // De no ser así ...
		{
			$pdf->Image($destino_foto6_nuevo, 160, 191.3, 30);                                     // ... Agrega la imágen foto3 en el archivo pdf en forma vertical
		}
	}
	else if($foto4 && $foto5!=NULL)  // Si las variables $foto1 y $foto2 contienen imagenes, las dos imágenes se agregarán al archivo pdf
	{
		copy($ruta_foto4, $destino_foto4);
		copy($ruta_foto5, $destino_foto5);

		$obj_simpleimage->load($destino_foto4);
		$obj_simpleimage->resize($obj_simpleimage->getWidth()/4, $obj_simpleimage->getHeight()/4);
		$obj_simpleimage->save($destino_foto4_nuevo);
		...
		$obj_simpleimage->load($destino_foto5);
		$obj_simpleimage->resize($obj_simpleimage->getWidth()/4, $obj_simpleimage->getHeight()/4);
		$obj_simpleimage->save($destino_foto5_nuevo);
	...
	else if($foto4 && $foto6!=NULL) // Si las variables $foto1 y $foto3 contienen imagenes, las dos imágenes se agregarán al archivo pdf
	{
		copy($ruta_foto4, $destino_foto4);
		copy($ruta_foto6, $destino_foto6);
		...
		$obj_simpleimage->load($destino_foto6);
		$obj_simpleimage->resize($obj_simpleimage->getWidth()/4, $obj_simpleimage->getHeight()/4);
		$obj_simpleimage->save($destino_foto6_nuevo);
	...
	else if($foto5 && $foto6!=NULL) // Si las variables $foto2 y $foto3 contienen imagenes, las dos imágenes se agregarán al archivo pdf
	{
		copy($ruta_foto5, $destino_foto5);
		copy($ruta_foto6, $destino_foto6);
	...
	if ($foto4!=NULL) 
	{
		unlink($destino_foto4);
		unlink($destino_foto4_nuevo);

		if($foto5!=NULL)
		{
			unlink($destino_foto5);
			unlink($destino_foto5_nuevo);

			if($foto3!=NULL)
			{
				unlink($destino_foto6);
				unlink($destino_foto6_nuevo);
			}
		} 
	}
	else if ($foto5!=NULL) 
