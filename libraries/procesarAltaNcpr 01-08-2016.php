<?php
	...
	if ($objCon->conectar()==true)
	{
		...
		$id_Folio=(mysql_result($result0, 0)+000001);
		...
		if($FECHAENVIO!="NULL")
		{
			$f_dia_envio=substr($FECHAENVIO,  0, 2);
			$f_mes_envio=substr($FECHAENVIO,  3, 2);
			$f_anio_envio=substr($FECHAENVIO, 6, 4);

			$FECHAENVIO_2=$f_anio_envio."-".$f_mes_envio."-".$f_dia_envio;
			$FECHAENVIO_3=$f_mes_envio."-".$f_dia_envio."-".$f_anio_envio;
		}
		else 
		{
		...
		$sql="INSERT INTO tbncprs VALUES('NULL', '$ORDEN', '$PARTE', '$TURNO', '$FECHAENVIO_2', '$DESCRIPCIONPARTE', '$PROVEEDOR', '$REPORTA', '$GENERA', '$FECHA', '$HORA', '$LOCALIZADEFECTO', '$LOCALIZADEFECTODESC', '$PIEZASORDEN', '$PIEZASRECHAZADAS', '$RECHAZO', '$RECHAZODESCRIPCION', '$DESCRIPCIONFALLA', '$LOCACIONDEFECTO', '$LOCACIONDEFECTODESC', '$DISPOSICION', '$DISPOSICIONDESCRIPCION', '$DETECCION', '$DETECCIONDESCRIPCION', '$ACCION', '$DESTINO')";
		$result=mysql_query($sql);	
		/*$id_Folio=mysql_insert_id(sql);		*/
	...
	$pdf->SetXY(155, 85);
	$pdf->SetTextColor(31, 73, 125); // Color Azul
	$pdf->cell(52, 5, $FECHAENVIO_3, 1, 1, 'C', 'F');
